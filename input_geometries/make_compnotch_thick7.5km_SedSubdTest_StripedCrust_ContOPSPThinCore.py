...
# geometry of weak crust section
stripe_width = 500.e3;
stripe_dist  = 500.e3;
...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		if x > (x_gap) and x <= (x_gap + x_SP - radius_outer - stripe_dist - stripe_width) and y > (ymax - y_crust):
			C[ind,2]=1
                elif x > (x_gap + x_SP - radius_outer - stripe_dist - stripe_width) and x <= (x_gap + x_SP - radius_outer - stripe_dist) and y > (ymax - y_crust):
                        C[ind,3]=1
                elif x > (x_gap + x_SP - radius_outer - stripe_dist) and x <= (x_gap + x_SP - radius_outer) and y > (ymax - y_crust):
                        C[ind,2]=1
		...
		# slab tracers
		if C[ind,2] != 1 and C[ind,3] != 1 and C[ind,4] != 1:
